...
from users.form import LocationForm
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if listing_form.is_valid() and location_form.is_valid():
                listing_form.save()
                location_form.save()
                messages.info(request, f'Listing {id} Successfully Updated')
                return redirect('home')
            else:
        ...
        messages.error(request, 'An error occured while accessing the edit page')
        return redirect('home')
